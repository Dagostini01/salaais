...
import { ScrollView, TouchableOpacity, Modal, View, Text, Alert, ActivityIndicator } from 'react-native';
...
import { Question as QuizQuestion } from './types';
...
export function Quiz() {
    ...
    useEffect(() => {
        async function fetchQuestions() {
            try {
                ...
                const formattedQuestions: QuizQuestion[] = quizData.data.map((question: any) => ({
                    id: question.id,
                    question: question.questao_texto,
                    bloco: question.bloco,
                    materia: question.materia,
                    answers: [
                        { id: 'a', text: question.questao_a, correct: question.alternativa_correta === 'a' },
                        { id: 'b', text: question.questao_b, correct: question.alternativa_correta === 'b' },
                        { id: 'c', text: question.questao_c, correct: question.alternativa_correta === 'c' },
                        { id: 'd', text: question.questao_d, correct: question.alternativa_correta === 'd' },
                    ],
                }));
    ...
    const handleFinishQuiz = async () => {
        ...
        try {
            await AsyncStorage.setItem('lastQuizResult', JSON.stringify(calculatedScorePercentage));
        } catch (error) {
    ...
    const filteredQuestions = selectedBlock !== null
        ? questions.filter(question => question.bloco === selectedBlock).slice(0, 20)
        : [];
                ...
                <>
                    ...
                    >
                        <ModalContainer style={{ alignItems: 'center' }}>
                            <Text style={{ fontSize: 16, marginBottom: 20 }}>Resultado final simulado ANAC:</Text>
                            <CircularProgress
                                size={120}
                                width={15}
                            ...
                            >
                                {() => (
                                    <Text style={{ fontSize: 24, color: '#000' }}>{`${scorePercentage.toFixed(0)}%`}</Text>
                                )}
                            </CircularProgress>

                            <View style={{ flexDirection: 'column', marginTop: 30, justifyContent: 'space-between' }}>
                                <ModalButton onPress={handleRestartQuiz} style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <MaterialIcons name="refresh" size={24} color="gray" style={{ marginRight: 8 }} />
                                    <ModalButtonText>Refazer</ModalButtonText>
                                </ModalButton>
                                <ModalButton onPress={handleReviewQuiz} style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <MaterialIcons name="assignment" size={24} color="gray" style={{ marginRight: 8 }} />
                                    <ModalButtonText>Revisão</ModalButtonText>
                                </ModalButton>
                                <ModalButton onPress={handleShowInfo} style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <MaterialIcons name="info" size={24} color="gray" style={{ marginRight: 8 }} />
                                    <ModalButtonText>Info</ModalButtonText>
                                </ModalButton>
                                <ModalButton onPress={handleCancelQuiz} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: theme.colors.attention }}>
                                    <MaterialIcons name="cancel" size={24} color="white" style={{ marginRight: 8 }} />
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginBottom: 20 }}>
                                {[1, 2, 3, 4].map(blockNumber => (
                                    <TouchableOpacity
                                        ...
                                        style={{
                                            padding: 10,
                                            backgroundColor: selectedBlock === blockNumber ? theme.colors.primary : 'gray',
                                            borderRadius: 10,
                                            flex: 1,
                                            alignItems: 'center',
                                            marginHorizontal: 5,
                                    ...
                                    >
                                        <Text style={{ color: 'white', fontSize: 16 }}>Bloco {blockNumber}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                                ...
                                {filteredQuestions.map((questionData, index) => (
                                    <QuizAnac key={questionData.id}>
                                        <Bloco style={{
                                            ...
                                            BL {questionData.bloco} - ({questionData.materia.toUpperCase()})
                                        </Bloco>
                                        <Question style={{
                                            padding: 10,
                                            marginVertical: 5,
                                        ...
                                        {questionData.answers.map((answer) => {
                                            const isSelected = selectedAnswers[String(questionData.id)] === answer.id;
                                            return (
                                                <TouchableOpacity
                                                    key={answer.id}
